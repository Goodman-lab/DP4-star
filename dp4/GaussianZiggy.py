...
def RunBatchOnZiggy(findex, queue, GausFiles, settings):
    ...
    outp = subprocess.check_output('ssh ziggy squeue', shell=True)
...
def WriteSubScript(GausJob, queue, ZiggyJobFolder, settings):
    ...
    QSub.write('HEADSCRATCH=/nodescratch/$(hostname)/' + settings.user + '/' + GausJob[:-4] + '\n')

    QSub.write('ln -s $HEADSCRATCH/$outfile $HERE/$outfile\n')
    QSub.write('srun $exe > $outfile < $inpfile\n')

    #Cleanup
    QSub.write('rm $HERE/$outfile\n')
    QSub.write('cp $SCRATCH/$outfile $HERE/$outfile\n')
...
def CheckZiggyQueue(JobIDs, settings):
    ...
    outp = subprocess.Popen(['ssh', 'ziggy', 'squeue', '-u ' + settings.user], \
