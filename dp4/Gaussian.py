...
def ReadFrequencies(GOutpFile):
    ...
    with open(GOutpFile, 'r') as gausfile:
        Goutp = gausfile.readlines()
    
    strt = 0
    frequencies = []
    intensities = []
    while not 'Harmonic frequencies (cm**-1), IR intensities (KM/Mole), Raman scattering' in Goutp[strt]:
        strt += 1
    # Set strt to the start of the frequency section
    strt += 6
    
    for line in Goutp[strt:]:
        if 'Frequencies' in line:
            data = [freq for freq in line.strip().split(' ') if freq]
            for idx in range(2, 5):
                frequencies.append(data[idx])
        if 'IR Inten' in line:
            data = [intensity for intensity in line.strip().split(' ') if intensity]
            for idx in range(3, 6):
                intensities.append(data[idx])
