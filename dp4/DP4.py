        ...
        if Settings.StatsParamFile == 'none':
            ...
            for errors in DP4data.Herrors:
                if errors:
                    ...
                    # also record MAE for proton
                    sum_absolute_errors = sum(abs(e) for e in errors)
                    DP4data.mae_H.append(sum_absolute_errors / len(errors))
...
def CalcDP4(DP4data, Isomers, Settings, load=False):
    ...
    # Display H molecular probabilities
    print('Molecular probabilities:')
    for i, prob in enumerate(DP4data.HDP4probs):
        print(f'Isomer {i + 1}: {prob}')
    ...
    if Settings.matching_assignment: 
        scaled_probs = []
        A = Settings.A
        for iso, prob in zip(Isomers, DP4data.HDP4probs):
            scaling_factor = math.exp(-A * iso.number_excluded)
            scaled_prob = prob * scaling_factor
            scaled_probs.append(scaled_prob)
        DP4data.HDP4probs = scaled_probs
        print('Scaled molecular probabilities:')
        for i, prob in enumerate(DP4data.HDP4probs):
            print(f'Isomer {i + 1}: {prob}')
...
def MakeOutput(DP4Data, Isomers, Settings):
    ...
    for i, mae in enumerate(DP4Data.mae_H):
        DP4Data.output += (f'\nIsomer {str(i + 1)}: {mae}')
...
def SaveResults(DP4Data, Settings):
    # output results in csv format to specified file

    molecule = os.getcwd().split('/')[-1]
    choice_dict = {
        'carbon': [molecule] + [':'.join(Settings.InputFiles)] + DP4Data.CDP4probs,
        'proton': [molecule] + [':'.join(Settings.InputFiles)] + DP4Data.HDP4probs,
        'combined': [molecule] + [':'.join(Settings.InputFiles)] + DP4Data.DP4probs
    }
    
    results_path = Settings.save_path

    with open(results_path, 'a') as results:
        writer_object = writer(results)
        writer_object.writerow(choice_dict[Settings.save_choice])
